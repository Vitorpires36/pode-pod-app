...
export interface BairroSP {
  ...
  valorBase: number; // estimativa média Uber Moto
}
...
// Lucro garantido por entrega
export const MARGEM_FIXA_ENTREGA = 10;

// Proteção contra dinâmica da Uber
export const BUFFER_UBER = 8;

// Valor máximo aceitável para o cliente (hard cap)
export const VALOR_MAXIMO_ENTREGA = 45;
...
// CÁLCULO FINAL
// =====================
export function calcularValorEntrega(bairro: BairroSP): number {
  ...
  return Math.min(
    Number(valorCalculado.toFixed(2)),
    VALOR_MAXIMO_ENTREGA
  );
}
